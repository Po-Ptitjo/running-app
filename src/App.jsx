...
export default function App() {
  ...
  const {
    ...
    generateCycle, updateSettings, resetData,
    exportData, importData, getProgressStats,
  } = useTraining()
  ...
  return (
    <div className="flex flex-col h-full bg-grid" style={{ background: '#07090F' }}>
      {/* App header */}
      <header className="flex-shrink-0 flex items-center justify-between px-4 pt-safe"
        style={{
      ...
      <main className="flex-1 overflow-y-auto scrollbar-hide" style={{ paddingBottom: '80px' }}>
        {activeTab === 'program' && (
          <ProgramView {...commonProps} />
        )}
        {activeTab === 'today' && (
          <TodayView {...commonProps} />
        )}
        {activeTab === 'progress' && (
